<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta http-equiv="X-UA-Compatible" content="ie=edge" />
    <title>YourStore</title>
    <link rel="stylesheet" href="index.css" />
    <link
      rel="stylesheet"
      media="screen and (max-width: 1170px)"
      href="css/phone.css"
    />
    <link
      href="https://fonts.googleapis.com/css?family=Baloo+Bhai|Bree+Serif&display=swap"
      rel="stylesheet"
    />
    <link
      rel="stylesheet"
      href="https://cdnjs.cloudflare.com/ajax/libs/font-wesome/6.1.1/css/all.min.css"
    />
  </head>

  <body>
    <nav id="navbar">
      <div id="logo">
        <img src="logo.jpg" alt="YourStore.com" />
      </div>
      <ul>
      <li class="item"><a href="home.php">Home</a></li>
        <li class="item"><a href="notebook.php">NOTEBOOKS</a></li>
        <li class="item"><a href="pens.php">PENS & PENCILS</a></li>
        <li class="item"><a href="instruments.php">INSTRUMENTS</a></li>
        <li class="item"><a href="tapes.php">TAPES AND GUMS</a></li>
      </ul>
      <div id="search-box">
        <input type="text" class="search-txt" placeholder="Type to Search">
        <a href=""></a>
        <div class="cart1"><i class="fa-solid fa-cart-arrow-down"></i></div>
      </div>
    </nav>
    <section id="notebooklist">
      <div class="header">
      <h1 class="h-primary center">TAPES AND GUMS</h1>
      <h2 class="h-secondary center">Broken things can be joined again</h2>
    </div>
      <div id="notebook">
        <div class="box">
          <h2 class="h-secondary center">COMPANY1</h2>
          <div class="company1">
            <div class="notebooklist">
              <img src="/photos/spiral.jpg" alt="" /> <b>Masking Tape</b>
              <div class="desc">Company 1 183 pages spiral notebook</div>
              <div class="rating">
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
              </div>
              <div><b>&#8377 100</b></div>
              <!-- <div class="counter">
                <button onclick="decrement()">-</button>
                <nbsp></nbsp>
                <h2 id="root1">0</h2>
                <nbsp></nbsp>
                <button onclick="increment()">+</button>
              </div> -->
              <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
            </div>
            <div class="notebooklist">
              <img src="/photos/notepad.jpg" alt="" /> <b>Electric tape</b>
              <div class="desc">Company 1 183 pages spiral notebook</div>
              <div class="rating">
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
              </div>
              <div><b>&#8377 100</b></div>
              <!-- <div class="counter">
                <button onclick="decrement()">-</button>
                <nbsp></nbsp>
                <h2 id="root2">0</h2>
                <nbsp></nbsp>
                <button onclick="increment()">+</button>
              </div> -->
              <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
            </div>
            <div class="notebooklist">
              <img src="/photos/diary.jpg" alt="" /> <b>Packing Tape</b>
              <div class="desc">Company 1 183 pages spiral notebook</div>
              <div class="rating">
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
              </div>
              <div><b>&#8377 100</b></div>
              <!-- <div class="counter">
                <button onclick="decrement()">-</button>
                <nbsp></nbsp>
                <h2 id="root3">0</h2>
                <nbsp></nbsp>
                <button onclick="increment()">+</button>
              </div> -->
              <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
            </div>
            <div class="notebooklist">
              <img src="/photos/composition.jpg" alt="" /> <b>Double Sided Tape</b>
              <div class="desc">Company 1 183 pages spiral notebook</div>
              <div class="rating">
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
                <i class="fas fa-star"></i>
              </div>
              <div><b>&#8377 100</b></div>
              <!-- <div class="counter">
                <button onclick="decrement()">-</button>
                <nbsp></nbsp>
                <h2 id="root4">0</h2>
                <nbsp></nbsp>
                <button onclick="increment()">+</button>
              </div> -->
              <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
            </div>
          </div>
        </div>
        <div id="notebook">
          <div class="box">
            <h2 class="h-secondary center">COMPANY1</h2>
            <div class="company1">
              <div class="notebooklist">
                <img src="/photos/spiral.jpg" alt="" /> <b>Masking Tape</b>
                <div class="desc">Company 1 183 pages spiral notebook</div>
                <div class="rating">
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                </div>
                <div><b>&#8377 100</b></div>
                <!-- <div class="counter">
                  <button onclick="decrement()">-</button>
                  <nbsp></nbsp>
                  <h2 id="root1">0</h2>
                  <nbsp></nbsp>
                  <button onclick="increment()">+</button>
                </div> -->
                <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
              </div>
              <div class="notebooklist">
                <img src="/photos/notepad.jpg" alt="" /> <b>Electric Tape</b>
                <div class="desc">Company 1 183 pages spiral notebook</div>
                <div class="rating">
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                </div>
                <div><b>&#8377 100</b></div>
                <!-- <div class="counter">
                  <button onclick="decrement()">-</button>
                  <nbsp></nbsp>
                  <h2 id="root2">0</h2>
                  <nbsp></nbsp>
                  <button onclick="increment()">+</button>
                </div> -->
                <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
              </div>
              <div class="notebooklist">
                <img src="/photos/diary.jpg" alt="" /> <b>Flooring Tape</b>
                <div class="desc">Company 1 183 pages spiral notebook</div>
                <div class="rating">
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                </div>
                <div><b>&#8377 100</b></div>
                <!-- <div class="counter">
                  <button onclick="decrement()">-</button>
                  <nbsp></nbsp>
                  <h2 id="root3">0</h2>
                  <nbsp></nbsp>
                  <button onclick="increment()">+</button>
                </div> -->
                <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
              </div>
              <div class="notebooklist">
                <img src="/photos/composition.jpg" alt="" /> <b>Double sided tape</b>
                <div class="desc">Company 1 183 pages spiral notebook</div>
                <div class="rating">
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                  <i class="fas fa-star"></i>
                </div>
                <div><b>&#8377 100</b></div>
                <!-- <div class="counter">
                  <button onclick="decrement()">-</button>
                  <nbsp></nbsp>
                  <h2 id="root4">0</h2>
                  <nbsp></nbsp>
                  <button onclick="increment()">+</button>
                </div> -->
                <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
              </div>
            </div>
          </div>
          <div id="notebook">
            <div class="box">
              <h2 class="h-secondary center">COMPANY1</h2>
              <div class="company1">
                <div class="notebooklist">
                  <img src="/photos/spiral.jpg" alt="" /> <b>Masking Tape</b>
                  <div class="desc">Company 1 183 pages spiral notebook</div>
                  <div class="rating">
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                  </div>
                  <div><b>&#8377 100</b></div>
                  <!-- <div class="counter">
                    <button onclick="decrement()">-</button>
                    <nbsp></nbsp>
                    <h2 id="root1">0</h2>
                    <nbsp></nbsp>
                    <button onclick="increment()">+</button>
                  </div> -->
                  <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
                </div>
                <div class="notebooklist">
                  <img src="/photos/notepad.jpg" alt="" /> <b>Flooring Tape</b>
                  <div class="desc">Company 1 183 pages spiral notebook</div>
                  <div class="rating">
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                  </div>
                  <div><b>&#8377 100</b></div>
                  <!-- <div class="counter">
                    <button onclick="decrement()">-</button>
                    <nbsp></nbsp>
                    <h2 id="root2">0</h2>
                    <nbsp></nbsp>
                    <button onclick="increment()">+</button>
                  </div> -->
                  <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
                </div>
                <div class="notebooklist">
                  <img src="/photos/diary.jpg" alt="" /> <b>Electric Tape</b>
                  <div class="desc">Company 1 183 pages spiral notebook</div>
                  <div class="rating">
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                  </div>
                  <div><b>&#8377 100</b></div>
                  <!-- <div class="counter">
                    <button onclick="decrement()">-</button>
                    <nbsp></nbsp>
                    <h2 id="root3">0</h2>
                    <nbsp></nbsp>
                    <button onclick="increment()">+</button>
                  </div> -->
                  <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
                </div>
                <div class="notebooklist">
                  <img src="/photos/composition.jpg" alt="" /> <b>Double sided tape</b>
                  <div class="desc">Company 1 183 pages spiral notebook</div>
                  <div class="rating">
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                    <i class="fas fa-star"></i>
                  </div>
                  <div><b>&#8377 100</b></div>
                  <!-- <div class="counter">
                    <button onclick="decrement()">-</button>
                    <nbsp></nbsp>
                    <h2 id="root4">0</h2>
                    <nbsp></nbsp>
                    <button onclick="increment()">+</button>
                  </div> -->
                  <a href="#"><i class="fa-solid fa-cart-arrow-down cart"></i></a>
                </div>
              </div>
            </div>
      </div>
    </section>
  </body>
</html>